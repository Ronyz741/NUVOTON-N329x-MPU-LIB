...
/*
	Write 16 Bits data to GPIOx
	GPIOx: GPIOA...B...E
	val: any 16	Bits data
	Example: GPIO_Write(GPIOA,0xFFFF)
*/
...
/*
	Set the GPIO which are choiced
	GPIOx: GPIOx: GPIOA...B...E
	GPIO_PIN: GPIO_Pin_0...GPIO_Pin_15
	Example: GPIO_SetBits(GPIOA,GPIO_Pin_0|GPIO_Pin_1)
*/
void GPIO_SetBits(GPIO_TypeDef* GPIOx,u16 GPIO_PIN)
{
	GPIOx->OUT = GPIOx->OUT | GPIO_PIN;
	
}

/*
	Reset the GPIO which are choiced
	GPIOx: GPIOx: GPIOA...B...E
	GPIO_PIN: GPIO_Pin_0...GPIO_Pin_15
	Example: GPIO_ResetBits(GPIOA,GPIO_Pin_0|GPIO_Pin_1)
*/
...
{
	return GPIOx->OUT & GPIO_PIN ? 1:0;
	
}

u8 GPIO_ReadInputDataBit(GPIO_TypeDef* GPIOx,u16 GPIO_PIN)
{
	return GPIOx->IN & GPIO_PIN ? 1:0;
...
{
	u16 pin_val;
	pin_val = GPIO_InitStructure->GPIO_Pin;
	if(GPIO_InitStructure->GPIO_Mode == GPIO_Mode_Out)
		GPIOx->MODE_OUT |= pin_val;
	else
		GPIOx->MODE_OUT &= ~pin_val;
	if(GPIO_InitStructure->GPIO_PuPd == GPIO_Pupd_None)
		GPIOx->PUEN &= ~pin_val;
	else if(GPIO_InitStructure->GPIO_PuPd ==  GPIO_Pupd_PU)
		GPIOx->PUEN |= pin_val;
